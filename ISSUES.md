...
- **Codex: NPC dossiers + monster bestiary** (`src/codex/`): two engine-owned Log sections, **People** and **Bestiary**, alongside Quests/Notes. Numeric knowledge (per-`definition_id` tiers + kill tallies) lives in `CharacterStash["codex"]` (`CodexState`); prose is composed server-side (`codex::compose`) and upserted as engine `LogEntry`s by the single writer `apply_codex_updates`, so persistence, replication, and per-entry player notes come free. Both reveal paths queue into `PendingCodexUpdates` (they hold `iter_mut` NPC borrows and can't also take `&mut CharacterStash`); ordering across three plugins is held by the `CodexSet::{Reveal, Apply}` sets, not `.before(fn)`. **People**: the existing Details verb now returns a structured `NpcDossier` (`GameUiEvent::OpenSocialRead`) with a 4-rung margin ladder — identity at tier 0 even on a *failed* check, then bearing / crimes / factions / lore — rendered as sections with a "sharper read might reveal more" hint (`src/ui/social_read.rs`). Only the durable half is filed to the log; bearing and crime knowledge stay live. **Bestiary**: `npc::bestiary::tick_bestiary_observation` rolls Perception every 3s per (player, creature type) within 12 tiles (skipping unspotted `Hidden` creatures), DC `8 + level + 2·(tier−1)`, one rung per roll, rescheduling on failure too; the Mastered rung also needs 10 kills, credited from `combat::damage` on the same `xp_credit()` attribution. Numbers are blurred on purpose (a vitality band, not the `hp` expression; "often", not `0.6`). New YAML: `occupation`, `lore`, `codex: people|bestiary|none` (with a `npc_behavior`/`dialog_node`/`shopkeeper`/`judge` fallback heuristic), `faction_display_names` (feeds `FactionInterner::build_with_display_names`). Drive-bys: `SetQuestPlayerNotes` was hard-wired to the Quests section, so notes on any other engine entry were silently rejected — generalized to `SetLogPlayerNotes`; `populate_bookmark_column` hardcoded two tabs — now driven by `log::PINNED_SECTIONS` with a scrollable column; and the `protocol_roundtrip` coverage guards were missing `RequestSocialRead`/`OpenSocialRead` (the tests didn't compile).

- **Loot content batch + item values + selling to vendors**: every item definition may now declare `value_copper` (its shelf price; players sell at half, Persuasion-modified via the previously-unused `TradeSide::PlayerSells`). Selling needs no new command — it is the ordinary shop trade, with `commit_player_to_shop_trade` crediting non-coin entries in the player's column instead of taking them for free, paying the balance as minted change (`currency::deposit_copper`, now `pub(crate)`), and shelving the sold goods on the merchant's wares list (`StockEntry::restock`). The preview number on the trade panel comes from the same `offer_credit_copper` the commit uses, computed server-side into `ClientTradeView::sale_credit_copper`, so display and payout cannot drift; the item Details window gained a "Sells for" row. **Content**: 24 new definitions (7 foods, 3 direct-restore potions — nothing outside the `hollow_bell` module used `restore_health`/`restore_mana` before — and 14 sell-fodder trinkets), sprites from `scripts/gen_loot_items_sprites.py`, and `value_copper` backfilled onto 70 existing items against the `content_bible.md` §9.1 ladder (coins and quest items deliberately left unvalued). **Drop tables** rewritten for the 11 core creatures + guards/townsfolk/villager + 5 `hollow_bell` trash mobs: `rat`, `skeleton` and `cyclops` previously had no `loot:` block at all and so left *no corpse*; coin denomination now follows the §10 tier table and only lands on creatures that would carry a purse (the fire elemental's purse is gone — it drops slag); elite gear odds trimmed to ~0.10–0.12 per piece. **Bug fixed on the way**: `QuantityDistribution::roll` read the wall clock with no salt, so every `uniform` drop resolved in one call returned an identical quantity — it now takes a per-drop salt (`world::loot::drop_salt`), and the quantity salt is offset off the probability salt so "did it drop" isn't tied to "how many". New guard test `every_loot_drop_names_a_real_definition` fails the build on a dangling drop `type_id`, which was previously silent.
