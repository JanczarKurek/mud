...
use crate::network::resources::{TcpClientConfig, TcpClientConnection, TcpServerConfig, TcpServerState};
...
impl Plugin for TcpClientPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(Update, poll_tcp_client_messages.before(apply_game_events_to_client_state));
    }
}
...
impl Plugin for TcpServerPlugin {
    fn build(&self, app: &mut App) {
        ...
        .add_systems(Update, poll_tcp_server_messages.before(process_game_commands))
        .add_systems(Update, flush_server_messages.after(apply_game_events_to_client_state));
    }
}
