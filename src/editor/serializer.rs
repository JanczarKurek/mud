...
use crate::editor::resources::EditorContext;
use crate::world::components::{OverworldObject, SpaceResident, TilePosition};
use crate::world::map_layout::{SpaceDefinitions, SpacePermanence, TileCoordinate};
use crate::world::object_registry::ObjectRegistry;

/// Serializable form of a space definition for writing back to YAML.
/// Only contains fields we want to write; leaves out the runtime fields.
#[derive(Serialize)]
struct SpaceOutput {
    authored_id: String,
    width: i32,
...
pub fn serialize_and_save(
    ...
    // Collect all (object_id, type_id, properties, tile) for this space.
    ...
    for (obj, resident, tile) in objects.iter() {
        ...
        items.push((
            ...
            type_id,
            properties,
            TileCoordinate { x: tile.x, y: tile.y },
        ));
    }

    // Group objects with empty properties by type_id into anonymous groups.
    // Objects with non-empty properties become explicit entries.
    ...
    // Sort anonymous groups for deterministic output.
    ...
    // Sort explicit entries by object_id for deterministic output.
    explicit.sort_by_key(|e| e.id);
    for entry in explicit {
        object_entries.push(ObjectEntryOutput::Explicit(entry));
    }

    // Preserve portals from the original authored definition.
    let portals = space_definitions
        .get(&ctx.authored_id)
        .map(|def| {
            def.portals
                .iter()
