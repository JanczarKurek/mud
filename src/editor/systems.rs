...
use crate::world::setup::{instantiate_space, spawn_overworld_object, sprite_for_definition, world_visual_for_definition};
...
pub fn insert_editor_visuals_pub(
    ...
    insert_editor_visuals(entity_commands, asset_server, def, world_config, tile, camera);
}
...
fn insert_editor_visuals(
    ...
    let anchor_y_offset = if def.render.y_sort { -effective_size * 0.5 } else { 0.0 };
...
pub fn attach_editor_visuals(
    ...
    for (entity, obj, tile, resident) in &objects {
        ...
        let Some(def) = definitions.get(&obj.definition_id) else { continue };
...
pub fn handle_editor_camera_pan(
    ...
    if modal_state.active.is_some() || editor_state.palette_filter_focused { return; }

    let mut delta = Vec2::ZERO;
    if keyboard.pressed(KeyCode::KeyW) || keyboard.pressed(KeyCode::ArrowUp) { delta.y += 1.0; }
    if keyboard.pressed(KeyCode::KeyS) || keyboard.pressed(KeyCode::ArrowDown) { delta.y -= 1.0; }
    if keyboard.pressed(KeyCode::KeyA) || keyboard.pressed(KeyCode::ArrowLeft) { delta.x -= 1.0; }
    if keyboard.pressed(KeyCode::KeyD) || keyboard.pressed(KeyCode::ArrowRight) { delta.x += 1.0; }
...
pub fn handle_editor_zoom(
    ...
    if modal_state.active.is_some() { return; }
    for event in mouse_wheel.read() {
        let factor = if event.y > 0.0 { 1.15_f32 } else { 1.0 / 1.15_f32 };
...
pub fn sync_tile_transforms_editor(
    ...
    for (space_resident, tile_position, world_visual, mut transform, visual_offset) in &mut query {
        ...
        let anchor_y_offset = if world_visual.y_sort { -effective_size * 0.5 } else { 0.0 };
        ...
        transform.translation = Vec3::new(
            ...
            (tile_position.y as f32 - editor_camera.center.y) * effective_size + anchor_y_offset + entity_offset.y,
            z,
        );
...
fn cursor_to_tile(cursor: Vec2, window: &Window, world_config: &WorldConfig, camera: &EditorCamera) -> TilePosition {
    let effective_size = world_config.tile_size * camera.zoom_level;
...
pub fn handle_editor_left_click(
    ...
    if interactions.iter().any(|i| *i != Interaction::None) { return; }
    let Ok(window) = windows.single() else { return };
    let Some(cursor) = window.cursor_position() else { return };
    let tile = cursor_to_tile(cursor, window, &world_config, &editor_camera);
    if tile.x < 0 || tile.y < 0 || tile.x >= editor_context.map_width || tile.y >= editor_context.map_height { return; }
    ...
    if editor_state.current_tool == EditorTool::Portal {
        ...
        modal_state.text_fields = vec![
            ModalTextField { label: "Portal ID".into(), value: String::new(), placeholder: "portal_to_dungeon".into(), numeric_only: false },
            ModalTextField { label: "Destination Space ID".into(), value: String::new(), placeholder: "starter_cellar".into(), numeric_only: false },
            ModalTextField { label: "Destination Tile X".into(), value: String::new(), placeholder: "7".into(), numeric_only: true },
            ModalTextField { label: "Destination Tile Y".into(), value: String::new(), placeholder: "9".into(), numeric_only: true },
        ];
    ...
    let Some(ref type_id) = editor_state.selected_type_id.clone() else { return };
    let Some(def) = definitions.get(type_id) else { return };

    let object_id = object_registry.allocate_runtime_id(type_id.clone());
    let entity = spawn_overworld_object(&mut commands, &definitions, object_id, type_id, None, editor_context.space_id, tile);
...
pub fn handle_editor_right_click(
    ...
    if editor_state.current_tool == EditorTool::Portal {
        if let Some(idx) = portal_buffer.portals.iter().position(|p| {
            p.source.x == tile.x && p.source.y == tile.y
        }) {
    ...
    if let Some((entity, obj, _, _)) = hit {
        ...
        let type_id = object_registry.type_id(deleted_id).unwrap_or(&obj.definition_id).to_owned();
        let properties = object_registry.properties(deleted_id).cloned().unwrap_or_default();
        undo_stack.push_undo(UndoOp::Spawn { type_id, space_id: editor_context.space_id, tile, properties });
...
pub fn handle_editor_keyboard_input(
    ...
    if editor_state.palette_filter_focused {
        for event in keyboard_events.read() {
            ...
            match event.key_code {
                KeyCode::Escape => { editor_state.palette_filter_focused = false; }
                KeyCode::Backspace => { editor_state.palette_filter.pop(); }
                _ => {
                    if event.repeat { continue; }
                    match &event.logical_key {
                        Key::Character(ch) => { editor_state.palette_filter.push_str(ch.as_str()); }
                        Key::Space => { editor_state.palette_filter.push(' '); }
                        _ => {}
                    }
                }
            }
    ...
    let Some(editing_index) = prop_buffer.editing_index else { return };
    for event in keyboard_events.read() {
        if !event.state.is_pressed() { continue; }
        match event.key_code {
            KeyCode::Escape => {
            ...
            KeyCode::Backspace => { prop_buffer.edit_text.pop(); }
            _ => {
                if event.repeat { continue; }
                match &event.logical_key {
                    Key::Character(ch) => { prop_buffer.edit_text.push_str(ch.as_str()); }
                    Key::Space => { prop_buffer.edit_text.push(' '); }
                    _ => {}
                }
            }
        }
...
fn commit_edit(
    ...
    if let Some(object_id) = prop_buffer.object_id {
        let props = prop_buffer.entries.iter().filter(|(k, _)| !k.is_empty()).map(|(k, v)| (k.clone(), v.clone())).collect();
...
pub fn handle_editor_escape(
    ...
    if modal_state.active.is_some() { return; }
    if prop_buffer.editing_index.is_some() { return; }
    if editor_state.palette_filter_focused { return; }
...
pub fn open_file_dialog_shortcut(
    ...
    let ctrl = keyboard.pressed(KeyCode::ControlLeft) || keyboard.pressed(KeyCode::ControlRight);
    if !ctrl || !keyboard.just_pressed(KeyCode::KeyO) || modal_state.active.is_some() { return; }
...
pub fn open_save_as_shortcut(
    ...
    let ctrl = keyboard.pressed(KeyCode::ControlLeft) || keyboard.pressed(KeyCode::ControlRight);
    let shift = keyboard.pressed(KeyCode::ShiftLeft) || keyboard.pressed(KeyCode::ShiftRight);
    if !(ctrl && shift && keyboard.just_pressed(KeyCode::KeyS)) || modal_state.active.is_some() { return; }
...
pub fn open_new_map_dialog_impl(modal_state: &mut ModalState) {
    *modal_state = ModalState {
        ...
        text_fields: vec![
            ModalTextField { label: "Map ID".into(), value: String::new(), placeholder: "my_dungeon".into(), numeric_only: false },
            ModalTextField { label: "Width".into(), value: String::new(), placeholder: "32".into(), numeric_only: true },
            ModalTextField { label: "Height".into(), value: String::new(), placeholder: "24".into(), numeric_only: true },
            ModalTextField { label: "Fill Tile Type".into(), value: String::new(), placeholder: "grass".into(), numeric_only: false },
        ],
        ..default()
    };
}
...
pub fn process_modal_confirm(
    ...
    if !modal_state.confirm_triggered { return; }
    modal_state.confirm_triggered = false;

    let Some(kind) = modal_state.active else { return };
    match kind {
        ModalKind::FileOpen => {
            let Some(idx) = modal_state.selected_list_item else {
                modal_state.error_message = Some("Select a map first.".into()); return;
            };
            let Some(authored_id) = modal_state.list_items.get(idx).cloned() else { return };
            if editor_state.dirty && modal_state.error_message.is_none() {
                modal_state.error_message = Some("Unsaved changes — click Open again to discard.".into()); return;
            }
        ...
        ModalKind::SaveAs => {
            let authored_id = modal_state.text_fields.first().map(|f| f.value.trim().to_owned()).unwrap_or_default();
            if authored_id.is_empty() { modal_state.error_message = Some("Map ID cannot be empty.".into()); return; }
            if !authored_id.chars().all(|c| c.is_alphanumeric() || c == '_') {
                modal_state.error_message = Some("Map ID: letters, digits, underscores only.".into()); return;
            }
            modal_state.active = None;
            modal_state.error_message = None;
        ...
        ModalKind::NewMap => {
            let vals: Vec<String> = modal_state.text_fields.iter().map(|f| f.value.trim().to_owned()).collect();
            let authored_id = vals.first().cloned().unwrap_or_default();
            if authored_id.is_empty() || !authored_id.chars().all(|c| c.is_alphanumeric() || c == '_') {
                modal_state.error_message = Some("Map ID must be non-empty alphanumeric.".into()); return;
            }
            let width: i32 = match vals.get(1).and_then(|s| s.parse().ok()) {
                Some(v) if v > 0 && v <= 256 => v,
                _ => { modal_state.error_message = Some("Width must be 1–256.".into()); return; }
            };
            let height: i32 = match vals.get(2).and_then(|s| s.parse().ok()) {
                Some(v) if v > 0 && v <= 256 => v,
                _ => { modal_state.error_message = Some("Height must be 1–256.".into()); return; }
            };
            let fill_type = vals.get(3).cloned().unwrap_or_else(|| "grass".into());
            if definitions.get(&fill_type).is_none() {
                modal_state.error_message = Some(format!("Unknown fill tile '{fill_type}'.")); return;
            }
            modal_state.active = None;
            modal_state.error_message = None;
            modal_state.confirmed = Some(ModalConfirmed::NewMap { authored_id, width, height, fill_type });
        }
        ModalKind::PortalCreate => {
            ...
            if id.is_empty() { modal_state.error_message = Some("Portal ID required.".into()); return; }
            if dest_space_id.is_empty() { modal_state.error_message = Some("Destination Space ID required.".into()); return; }
            let Some(source_tile) = modal_state.portal_source_tile else { return };
            modal_state.active = None;
            modal_state.error_message = None;
            modal_state.confirmed = Some(ModalConfirmed::PortalCreate { source_tile, id, dest_space_id, dest_tile_x, dest_tile_y });
        }
    }
}
...
pub fn apply_modal_confirmed(
    ...
    let Some(confirmed) = modal_state.confirmed.take() else { return };
    ...
    match confirmed {
        ModalConfirmed::FileOpen { authored_id } => {
            ...
            let Some(def) = space_definitions.get(&authored_id).cloned() else { return };

            let space_id = if let Some(id) = space_manager.persistent_space_id(&authored_id) {
                id
            } else {
                instantiate_space(&mut commands, &mut space_manager, &def, &object_definitions, None, def.permanence)
            };
        ...
        ModalConfirmed::SaveAs { authored_id } => {
            editor_context.authored_id = authored_id.clone();
            serialize_and_save(&editor_context, &portal_buffer, &object_registry, &objects_save);
            space_definitions.load_single_from_disk(&authored_id);
            editor_state.dirty = false;
            info!("Saved map as '{authored_id}'");
        }
        ModalConfirmed::NewMap { authored_id, width, height, fill_type } => {
            ...
            space_definitions.insert_or_replace(
                crate::world::map_layout::SpaceDefinition::new_empty(authored_id.clone(), width, height, fill_type.clone()),
            );
        ...
        ModalConfirmed::PortalCreate { source_tile, id, dest_space_id, dest_tile_x, dest_tile_y } => {
            let portal = PortalDefinition {
                id,
                source: TileCoordinate { x: source_tile.x, y: source_tile.y },
                destination_space_id: dest_space_id,
                destination_tile: TileCoordinate { x: dest_tile_x, y: dest_tile_y },
...
pub fn sync_portal_overlays(
    ...
    if !portal_buffer.is_changed() && !editor_context.is_changed() { return; }
    for entity in &markers { commands.entity(entity).despawn(); }
    for (i, portal) in portal_buffer.portals.iter().enumerate() {
        ...
        commands.spawn((
            ...
            SpaceResident { space_id: editor_context.space_id },
            tile,
            WorldVisual { z_index: 8.0, y_sort: false, sprite_height: 0.0 },
            Sprite { color: Color::srgba(0.2, 0.6, 1.0, 0.55), custom_size: Some(Vec2::splat(48.0)), ..default() },
            Transform::default(),
        ));
    }
}
