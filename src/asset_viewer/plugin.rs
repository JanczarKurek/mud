...
use crate::asset_viewer::systems::{
    apply_clip_change, attach_preview_animation, handle_clip_button_clicks,
    handle_filter_click, handle_inspector_row_click, handle_keyboard, handle_palette_clicks,
    handle_save_button, handle_tab_clicks, handle_viewer_zoom, setup_viewer_camera,
    sync_clip_button_highlight, sync_clip_buttons, sync_filter_text, sync_inspector_panel,
    sync_palette, sync_save_button, sync_tab_buttons, sync_top_bar_title, update_preview,
};
...
impl Plugin for AssetViewerPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(OverworldObjectDefinitions::load_from_disk())
            ...
            .add_systems(
                Startup,
                (setup_viewer_camera, spawn_viewer_hud.after(setup_viewer_camera)),
            )
            .add_systems(
                Update,
                (
