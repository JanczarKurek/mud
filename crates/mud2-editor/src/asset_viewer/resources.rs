...
pub struct ViewerState {
    ...
    /// Bumped whenever assets are reloaded so dependent systems can re-fire
    /// even when the selection hasn't changed.
    pub reload_counter: u32,
}

/// Tracks paths that the inspector just wrote to disk. The file watcher consults
/// this to suppress redundant reload events generated by our own saves.
#[derive(Resource, Default)]
...
impl SelfWriteSuppressor {
    pub fn expect<P: Into<PathBuf>>(&mut self, path: P) {
        self.pending.insert(path.into(), Instant::now());
    }

    /// If the watcher reports a change at `path` and we caused it, returns
    /// true and removes the entry. Returns false if the change was external.
    pub fn consume(&mut self, path: &Path) -> bool {
        ...
        if let Some(entry_path) = self
            ...
            .keys()
            .find(|p| {
                let candidate = canonicalize_or_self(p);
                candidate == canonical
            })
            .cloned()
        {
            self.pending.remove(&entry_path);
            return true;
        }
        false
    }
    ...
    pub fn purge_expired(&mut self) {
        let now = Instant::now();
        self.pending
            .retain(|_, t| now.duration_since(*t) < SELF_WRITE_TTL);
    }
}
...
pub struct InspectorBuffer {
    ...
    pub dirty: bool,
    /// Set when the watcher detects an external change to the currently-
    /// edited asset and `dirty == true`. Resolved by the conflict banner.
    pub conflict: bool,
}
...
impl InspectorBuffer {
    ...
    /// Persist the buffer to disk. Returns the path that was written so the
    /// caller can stamp it into `SelfWriteSuppressor`.
    pub fn save(&mut self) -> Result<PathBuf, String> {
        ...
        let path: PathBuf = match self.kind {
            AssetKind::Object => format!("assets/overworld_objects/{}/metadata.yaml", id).into(),
            AssetKind::Spell => format!("assets/spells/{}.yaml", id).into(),
        };
