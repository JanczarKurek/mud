...
use mud2::game::resources::PendingGameCommands;
use mud2::npc::components::SpawnGroupMember;
use mud2::npc::spawn_groups::SpawnGroupRegistry;
use mud2::player::components::Player;
use mud2::world::animation::VisualOffset;
use mud2::world::components::{
    Container, OverworldObject, Quantity, SpaceId, SpaceResident, TilePosition, ViewPosition,
    WorldVisual,
};
use mud2::world::floor_definitions::{FloorTilesetDefinitions, FloorTypeId};
use mud2::world::floor_map::FloorMaps;
...
use mud2::world::object_definitions::{OverworldObjectDefinition, OverworldObjectDefinitions};
use mud2::world::object_registry::ObjectRegistry;
use mud2::world::resources::{RuntimeSpace, SpaceManager};
use mud2::world::setup::{build_object_visual_bundle, instantiate_space, spawn_overworld_object};
use mud2::world::WorldConfig;
...
pub fn reset_space_contents_from_def(
    ...
    space_id: SpaceId,
    def: &mud2::world::map_layout::SpaceDefinition,
    ...
    for object in &def.resolved_objects {
        ...
        mud2::world::setup::spawn_overworld_object_instance(
            commands,
            object_definitions,
            object_registry,
            def,
...
pub fn init_editor_context(
    ...
    commands.insert_resource(EditorContext {
        ...
        permanence: def
            .map(|d| d.permanence)
            .unwrap_or(mud2::world::map_layout::SpacePermanence::Persistent),
...
pub fn sync_tile_transforms_editor(
    ...
    editor_context: Res<EditorContext>,
    editor_state: Res<EditorState>,
    visible_range: Res<mud2::world::floors::VisibleFloorRange>,
    mut query: Query<
        (
            &SpaceResident,
            &TilePosition,
    ...
    for (space_resident, tile_position, world_visual, mut transform, visual_offset) in &mut query {
        ...
        let object_floor = mud2::world::components::floor_index(tile_position.z);
        ...
        let z_sort = if !is_active {
            ...
            let floor = mud2::world::components::floor_index(tile_position.z);
            mud2::world::systems::y_sort_z(tile_position.x, tile_position.y, floor, 0, 0)
        } else {
            let floor = mud2::world::components::floor_index(tile_position.z);
            mud2::world::systems::flat_floor_z(world_visual.z_index, floor)
        };
        let floor_offset = if is_active {
            mud2::world::systems::floor_screen_offset(
...
pub fn update_editor_cursor_ghost(
    ...
    editor_state: Res<EditorState>,
    definitions: Res<OverworldObjectDefinitions>,
    floor_defs: Res<mud2::world::floor_definitions::FloorTilesetDefinitions>,
    panel_roots: crate::editor::ui::EditorPanelRoots,
    ...
    match editor_state.current_tool {
        EditorTool::Brush => {
            ...
            let z_base = if def.render.y_sort {
                mud2::world::systems::y_sort_z(tile.x, tile.y, active_floor_index, 0, 0)
            } else {
                mud2::world::systems::flat_floor_z(def.render.z_index, active_floor_index)
            };
            let z = z_base + 50.0;
...
fn commit_edit(
    ...
    if let Some(object_id) = prop_buffer.object_id {
        let mut props: mud2::world::map_layout::ObjectProperties = prop_buffer
            .entries
            .iter()
...
pub fn handle_editor_floor_brush_hotkey(
    keyboard: Res<ButtonInput<KeyCode>>,
    editor_keys: Res<mud2::ui::settings::EditorKeybindings>,
    modal_state: Res<ModalState>,
    mut editor_state: ResMut<EditorState>,
    floor_defs: Res<mud2::world::floor_definitions::FloorTilesetDefinitions>,
) {
    ...
    if editor_keys.just_pressed(mud2::ui::settings::EditorAction::ToolBrushLegacy, &keyboard) {
        editor_state.current_tool = EditorTool::Brush;
        return;
    }
    if editor_keys.just_pressed(mud2::ui::settings::EditorAction::ToolFloorCycle, &keyboard) {
...
pub fn handle_editor_save(
    ...
    if editor_keys.just_pressed(mud2::ui::settings::EditorAction::Save, &keyboard) {
        serialize_and_save(
            &editor_context,
            &portal_buffer,
            &spawn_group_buffer,
...
pub fn open_file_dialog_shortcut(
    ...
    if !editor_keys.just_pressed(mud2::ui::settings::EditorAction::OpenMap, &keyboard)
...
pub fn open_save_as_shortcut(
    ...
    if !editor_keys.just_pressed(mud2::ui::settings::EditorAction::SaveAs, &keyboard)
        || modal_state.active.is_some()
    {
        return;
    }
...
fn build_floor_picker(
    ...
    let mut sorted: Vec<&mud2::world::floor_definitions::FloorTilesetDefinition> =
...
pub fn sync_editor_lighting_to_world(
    ...
    editor_context: Res<EditorContext>,
    mut space_definitions: ResMut<SpaceDefinitions>,
    mut space_manager: ResMut<mud2::world::resources::SpaceManager>,
    mut client_state: ResMut<mud2::game::resources::ClientGameState>,
) {
...
pub fn init_editor_client_space(
    ...
    client_state.current_space = Some(mud2::game::resources::ClientSpaceState {
        space_id: editor_context.space_id,
...
pub fn sync_editor_view_to_client(
    editor_context: Res<EditorContext>,
    editor_camera: Res<EditorCamera>,
    world_clock: Res<mud2::world::lighting::WorldClock>,
    mut client_state: ResMut<mud2::game::resources::ClientGameState>,
) {
    ...
    if editor_camera.is_changed() || editor_context.is_changed() {
        let tile = mud2::world::components::TilePosition::ground(
            editor_camera.center.x.round() as i32,
            editor_camera.center.y.round() as i32,
        );
        let pos = mud2::world::components::SpacePosition::new(editor_context.space_id, tile);
    ...
    match confirmed {
        ...
        ModalConfirmed::NewMap {
            ...
            space_manager.insert_space(RuntimeSpace {
                ...
                permanence: mud2::world::map_layout::SpacePermanence::Persistent,
                instance_owner: None,
                lighting: mud2::world::map_layout::SpaceLightingDef::default(),
            });
            let new_def = mud2::world::map_layout::SpaceDefinition::new_empty(
                authored_id.clone(),
                width,
                height,
                fill_type.clone(),
            ...
            floor_maps.insert(
                new_space_id,
                mud2::world::components::TilePosition::GROUND_FLOOR,
                new_def.build_floor_map(mud2::world::components::TilePosition::GROUND_FLOOR),
            );
            ...
            editor_context.permanence = mud2::world::map_layout::SpacePermanence::Persistent;
            ...
            lighting_buffer.config = mud2::world::map_layout::SpaceLightingDef::default();
            lighting_buffer.selected_keyframe = None;
        ...
        ModalConfirmed::GenerateDungeon {
            ...
            let params = mud2::world::dungeon_gen::DungeonParams {
                width,
                height,
            ...
            let mut def = mud2::world::dungeon_gen::generate_dungeon(authored_id.clone(), params);
