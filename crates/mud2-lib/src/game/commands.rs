...
pub enum GameCommand {
    ...
    /// attached).
    SetHome,
    /// Toggle the player's sneaking state. While sneaking, movement is slower and
    /// quieter and the player rolls Stealth against NPC detection. Handled in
    /// `process_game_commands` by inserting/removing the `Sneaking` marker.
    SetSneaking {
        sneaking: bool,
    },
    ...
    /// Toggle the player's Auto-Retaliate state. While enabled, being attacked
    /// with no current combat target auto-locks one attacker. Handled in
    /// `process_game_commands` by inserting/removing the `AutoRetaliate` marker.
    SetAutoRetaliate {
        auto_retaliate: bool,
    },
